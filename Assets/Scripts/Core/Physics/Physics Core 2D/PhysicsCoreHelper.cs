...
public static class PhysicsCoreHelper
{
    ...
    public unsafe struct ShapeProxyForJobs
    {
        ...
        private readonly float2* vertex => (float2*)Unsafe.AsPointer(ref Unsafe.AsRef(in vertex0));
        //^Unsafe.AsRef allows us to get around readonly, so we can use the pointer in readonly properties/methods
        private readonly float2* normal => (float2*)Unsafe.AsPointer(ref Unsafe.AsRef(in normal0));
