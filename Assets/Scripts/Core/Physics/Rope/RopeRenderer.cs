...
public class RopeRenderer
{
    ...
    public void Initialize(int numNodes, int numEndcapTriangles)
    {
        ...
        bodyMaterial.SetInt(endcapTrianglesProperty, numEndcapTriangles);
        endcapMaterial.SetInt(numNodesProperty, numNodes);
        endcapMaterial.SetInt(endcapTrianglesProperty, numEndcapTriangles);
    ...
    public static Mesh CreateMesh(int numNodes, int endcapTriangles)
    {
        var vertices = new NativeArray<Vector3>(2 * numNodes + endcapTriangles, Allocator.TempJob);
        var uv = new NativeArray<Vector2>(vertices.Length, Allocator.TempJob);
        var bodyTris = new NativeArray<int>(6 * (numNodes - 1), Allocator.TempJob);
        var endcapTris = new NativeArray<int>(3 * endcapTriangles, Allocator.TempJob);
        ...
        var ropeMesh = new Mesh() { subMeshCount = 2 };
