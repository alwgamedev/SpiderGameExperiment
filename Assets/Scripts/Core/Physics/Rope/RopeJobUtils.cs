...
public static class RopeJobUtils
{
    ...
    public static (float2 pos, float2 lastPos, float2 velocity) MoveNodeFast(float2 position, float radius, float mass,
        float2 lastPosition, float2 movement, NativeArray<PhysicsCoreHelper.ShapeProxyForJobs> shapeCapture, PhysicsWorld world,
        PhysicsQuery.QueryFilter collisionFilter, PhysicsMask dynamicCollisionMask, float collisionBounciness, float dynamicCollisionForce)
    {
        ...
        var (pos, lastPos, velocity, _) = MoveNodeUntilImpact(position, radius, mass, lastPosition, movement, movement, OVERLAP_BUFFER, 
            dynamicCollisionMask, castResults, shapeCapture, collisionBounciness, dynamicCollisionForce);
    ...
    public static (float2 pos, float2 lastPos, float2 velocity) MoveNodeCareful(float2 position, float radius, float mass,
        float2 lastPosition, float2 movement, NativeArray<PhysicsCoreHelper.ShapeProxyForJobs> shapeCapture,
        ...
        float collisionBounciness, float dynamicCollisionForce)
    {
        ...
        var (pos, lastPos, velocity, _) = MoveNodeUntilImpact(position, radius, mass, lastPosition, movement, movement,
            OVERLAP_BUFFER, dynamicCollisionMask, castResults1, shapeCapture, collisionBounciness, dynamicCollisionForce);
        ...
        (pos, lastPos, velocity, _) = MoveNodeUntilImpact(pos, radius, mass, lastPos, velocity, remainingMovement, OVERLAP_BUFFER, 
            dynamicCollisionMask, castResults2, shapeCapture,  collisionBounciness, dynamicCollisionForce);
    ...
    public static (float2 pos, float2 lastPos, float2 velocity, float2 collisionNormal)
        ...
        PhysicsMask dynamicCollisionMask, NativeArray<PhysicsQuery.WorldCastResult> castResults, NativeArray<PhysicsCoreHelper.ShapeProxyForJobs> shapeCapture,
        float collisionBounciness, float dynamicCollisionForce)
    {
        if (castResults.Length == 0)
        {
        ...
        {
            float separation;
            (separation, collisionNormal1) = CollisionUtilities.SeparateCircleFromShape(pos, radius + overlapBuffer,
                shapeCapture[shapeId], result.shape.transform);
            ...
            if ((result.shape.contactFilter.categories & dynamicCollisionMask) != 0)
            {
    ...
    public static void MoveTerminusUnanchored(NativeArray<float2> position, float radius, float mass,
        NativeArray<float2> lastPosition, float2 movement, NativeArray<PhysicsCoreHelper.ShapeProxyForJobs> shapeCapture,
        NativeReference<PhysicsShape> terminusAnchor, NativeReference<float2> terminusAnchorLocalPos,
        NativeReference<FastRope.TerminusAnchorMode> terminusAnchorMode, PhysicsWorld world, PhysicsMask anchorMask,
        PhysicsQuery.QueryFilter collisionFilter, PhysicsMask dynamicCollisionMask, float collisionBounciness, 
        float dynamicCollisionForce, bool stepVelocity)
    {
        ...
        if (Hint.Unlikely(castResults.Length > 0 && (castResults[0].shape.contactFilter.categories & anchorMask) != 0))
        {
            position[^1] = pos;
    ...
    //         || shape.shapeType == PhysicsShape.ShapeType.Circle 
    //         || shape.shapeType == PhysicsShape.ShapeType.Capsule 
    //         || shape.shapeType == PhysicsShape.ShapeType.Polygon;
    // }


    //CONSTRAINTS

    public static float4 CalculateConstraint(float2 p0, float mass0, float2 p1, float mass1, float nodeSpacing,
        float nodeSpacing2, float constraintStiffness)
    {
        var d = p1 - p0;
