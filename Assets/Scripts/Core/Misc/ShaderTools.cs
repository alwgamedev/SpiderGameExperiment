...
{
    public static void SetBuffer(this ComputeShader computeShader, ComputeBuffer computeBuffer, int nameID, params int[] kernelIndices)
    {
        for (int i = 0; i < kernelIndices.Length; i++)
        {
            computeShader.SetBuffer(kernelIndices[i], nameID, computeBuffer);
        }
    }
