...
public struct GroundMapUpdate : IJob
{
    ...
    public GroundMapUpdate(NativeArray<float2> point, NativeArray<float2> normal, NativeArray<float> arcLengthPos,
    ...
    private MinimalHitData MapCircle(ref int i, ref int end, int sign, MinimalHitData hit)
    {
        ...
        var stepRotation = new PhysicsRotate() { direction = new float2(stepRotX, stepRotY) };
